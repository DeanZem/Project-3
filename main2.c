...
/* FIFO replacement: select next available frame */
frame_num = next;

/* invalidate the page that previously occupied this frame */
for (int i = 0; i < PAGE_COUNT; i++) {
if (page_tlb[i] == frame_num) {
page_tlb[i] = -1;
break;
}
}
...
/* move pointer to next frame in circular fashion */
